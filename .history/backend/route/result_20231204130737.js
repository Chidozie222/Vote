...
results.post('/voter_for_candidate', async(req, res) => {
    ...
    try {
                    let canName = await result.find({Useremail, Title, position, candidateName})
...
results.get('/results/:Useremail/:Title/:position/:candidateName', async(req, res) => {
    ...
    try {
        let user = await result.find({Useremail, Title, position, candidateName})
                    if (user) {
...
results.get('/remove/:Useremail/:Title/:email', async(req, res) => {
    ...
    try {
            let title = await remove.find({Useremail, Title})
        if (title.length > 0) {
                for (let index = 0; index < array.length; index++) {
                    if(title[index].email == email ){
