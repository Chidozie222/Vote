...
const button = () => {
    if (candidate.length > 0) {
        return (
            <button type="button" className="mission_impossible_dead-button" onClick={() => project_impossible(page + 1)}>
                Next
            </button>
        );
    }
...
const auth_tocatch_thebadguy = () => {
    if (candidate.length > 0) {
        return (
            <>
                <button type="button" onClick={()=> auth()}  className="voter_submit-button">submit</button>
            </>
        );
    }
}
        ...
        <div className="background">
            ...
            <div className="voting_paltform">
                ...
                <div className="voting_body">
                    {Position.map((title) => (
                    <p className="position_for-sider" key={title._id}>{title.position}</p>
                    ))}
                    <h1 id="result_Title">{Title}</h1>
                    <div className="contain_voting_paltform">
